...
ftplib::ftplib()
{
	...
	{
		LOG(ERROR) << "ftplib::ftplib calloc mp_ftphandle Failed";
	}
	mp_ftphandle->buf = static_cast<char *>(malloc(FTPLIB_BUFSIZ));
	if (mp_ftphandle->buf == NULL)
	{
		LOG(ERROR) << "ftplib::ftplib calloc mp_ftphandle->buf Failed";
		free(mp_ftphandle);
	}
	#ifndef NOSSL
...
int ftplib::readline(char *buf,int max,ftphandle *ctl)
{
	...
	do
	{
		...
		if ( x == -1)
		{
			LOG(ERROR) << "ftplib::readline net_read Failed";
			retval = -1;
			break;
		}
...
int ftplib::readresp(char c, ftphandle *nControl)
{
	...
	{
		LOG(ERROR) << "ftplib::readresp readline 1 Control socket read failed";
		return 0;
	}
	...
	if (nControl->response[3] == '-')
	{
		...
		do
		{
			if (readline(nControl->response,256,nControl) == -1)
			{
				LOG(ERROR) << "ftplib::readresp readline 2 Control socket read failed";
				return 0;
			}
...
int ftplib::Connect(const char *host)
{
	...
	if (pnum == NULL)
	{
		...
		{
			LOG(ERROR) << "ftplib::Connect getservbyname failed";
			free(lhost);
			return 0;
		}
...
#endif
	{
		...
		{
			LOG(ERROR) << "ftplib::Connect gethostbyname failed";
			free(lhost);
			return 0;
		}
	...
	if (sControl == -1)
	{
		LOG(ERROR) << "ftplib::Connect socket failed";
		return 0;
	}
	...
	if (setsockopt(sControl,SOL_SOCKET,SO_REUSEADDR, SETSOCKOPT_OPTVAL_TYPE &on, sizeof(on)) == -1)
	{
		...
		LOG(ERROR) << "ftplib::Connect setsockopt failed";
		net_close(sControl);
		return 0;
	}
	if (connect(sControl, (struct sockaddr *)&sin, sizeof(sin)) == -1)
	{
		LOG(ERROR) << "ftplib::Connect connect failed";
		net_close(sControl);
		return 0;
	}
...
int ftplib::FtpSendCmd(const char *cmd, char expresp, ftphandle *nControl)
{
	...
	if (x <= 0)
	{
		LOG(ERROR) << "ftplib::FtpSendCmd write failed";
		return 0;
	}
...
int ftplib::FtpOpenPort(ftphandle *nControl, ftphandle **nData, transfermode mode, int dir, char *cmd)
{
	...
	{
		LOG(ERROR) << "ftplib::FtpOpenPort getsockname failed";
		return -1;
	}
	...
	{
		LOG(ERROR) << "ftplib::FtpOpenPort bind failed";
		net_close(sData);
		return -1;
	}
	if (listen(sData, 1) < 0)
	{
		LOG(ERROR) << "ftplib::FtpOpenPort listen failed";
		net_close(sData);
		return -1;
	}
	...
	if (ctrl == NULL)
	{
		LOG(ERROR) << "ftplib::FtpOpenPort calloc ctrl failed";
		net_close(sData);
		return -1;
	}
	if ((mode == 'A') && ((ctrl->buf = static_cast<char*>(malloc(FTPLIB_BUFSIZ))) == NULL))
	{
		LOG(ERROR) << "ftplib::FtpOpenPort calloc ctrl->buf failed";
		net_close(sData);
		free(ctrl);
		return -1;
	}
...
int ftplib::FtpOpenPasv(ftphandle *nControl, ftphandle **nData, transfermode mode, int dir, char *cmd)
{
	...
	if (sData == -1)
	{
		LOG(ERROR) << "ftplib::FtpOpenPasv socket failed";
		return -1;
	}
	if (setsockopt(sData,SOL_SOCKET,SO_REUSEADDR, SETSOCKOPT_OPTVAL_TYPE &on,sizeof(on)) == -1)
	{
		LOG(ERROR) << "ftplib::FtpOpenPasv setsockopt SO_REUSEADDR failed";
		net_close(sData);
		return -1;
	}
	if (setsockopt(sData,SOL_SOCKET,SO_LINGER, SETSOCKOPT_OPTVAL_TYPE &lng,sizeof(lng)) == -1)
	{
		LOG(ERROR) << "ftplib::FtpOpenPasv setsockopt SO_LINGER failed";
		net_close(sData);
		return -1;
	}
	...
	if (ret <= 0)
	{
		LOG(ERROR) << "ftplib::FtpOpenPasv net_write failed";
		return -1;
	}
	...
	if (connect(sData, &sin.sa, sizeof(sin.sa)) == -1)
	{
		...
		LOG(ERROR) << "ftplib::FtpOpenPasv connect failed";
		net_close(sData);
		return -1;
	}
	...
	if (ctrl == NULL)
	{
		LOG(ERROR) << "ftplib::FtpOpenPasv calloc ctrl failed";
		net_close(sData);
		return -1;
	}
	if ((mode == 'A') && ((ctrl->buf = static_cast<char*>(malloc(FTPLIB_BUFSIZ))) == NULL))
	{
		LOG(ERROR) << "ftplib::FtpOpenPasv calloc ctrl->buf failed";
		net_close(sData);
		free(ctrl);
		return -1;
	}
	...
	else
	{
		while ((l = FtpRead(dbuf, FTPLIB_BUFSIZ, nData)) > 0)
		{
			if (fwrite(dbuf, 1, l, local) <= 0)
			{
				...
				LOG(ERROR) << "ftplib::FtpXfer localfile write failed";
				break;
			}
		}
	}
...
int ftplib::CorrectPasvResponse(unsigned char *v)
{
	...
	{
		LOG(ERROR) << "ftplib::CorrectPasvResponse getpeername failed";
		net_close(mp_ftphandle->handle);
		return 0;
	}
